...
### React状态管理与进阶
- 状态提取：将子状态从一个组件移动到其他组件中的重构过程被称为状态提取。
接收组件必须是es6类组件形式，才能管理状态。
- 要想在组件中管理状态,需要添加构造函数和初始状态.
- 相关逻辑代码和状态信息从App组件移动到了Table组件中,这使得App组件更加轻量,此外,Table的排序逻辑放在了组件内部,所以它的接口也更加轻量了.
- 状态提升:从子组件到父组件
可能情况比如:假设需要在子组件的兄弟组件上显示组件的状态,就需要将状态提升到父组件中,再向子组件暴露状态.
- setState()
  1. 可以给该函数传递一个对象来改变部分的内部状态
  2. 可以传入一个函数来更新状态
    **应用场景**:更新状态取决于之前的状态或者属性,如果不使用函数参数的形式,组件的内部状态管理可能会引起bug
    **原因**:setState()方法是异步执行的,React依次执行setState()方法,最终会全部执行完毕,如果setState()方法依赖于之前的状态或者属性,有可能在按批次执行的期间,状态或者属性的值就已经被改变了.**使用函数参数形式的话,传入setState()方法的参数是一个回调,该回调会在被执行时传入状态和属性,尽管setState()方法是异步的,但是通过回调函数,它使用的是执行那一刻的状态和属性.不明白这个啊!!!!!!**
